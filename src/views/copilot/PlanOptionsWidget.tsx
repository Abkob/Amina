...
function StatusPill({ option }: { option: ChatPlanOption }) {
  const tone =
    option.scheduler.status === 'feasible' ? 'text-emerald-300 border-emerald-400/25 bg-emerald-500/10'
      : option.scheduler.status === 'impossible' ? 'text-red-300 border-red-400/25 bg-red-500/10'
    ...
    <div className="mt-2 overflow-hidden rounded-2xl border border-white/10 bg-[#111120] shadow-xl shadow-black/20">
      <div className="border-b border-white/8 px-4 py-3">
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm font-bold text-white">{planOptions.title}</p>
            <p className="mt-0.5 text-[11px] leading-snug text-gray-400">{planOptions.summary}</p>
          </div>
          <span className="shrink-0 rounded-full border border-white/10 bg-white/5 px-2 py-1 font-mono text-[9px] font-bold uppercase text-gray-400">
      ...
      <div className="space-y-4 p-3">
        {planOptions.options.map((option, index) => (
          <section key={option.option_id} className="rounded-2xl border border-white/10 bg-white/[0.035] p-3">
            <div className="mb-2 flex flex-wrap items-start gap-2">
              <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-indigo-500/15 font-mono text-[10px] font-bold text-indigo-200">
                {index + 1}
              </span>
              <div className="min-w-0 flex-1">
                <p className="text-[13px] font-bold text-white">{option.name}</p>
                <p className="mt-0.5 text-[11px] leading-snug text-gray-400">{option.description}</p>
              </div>
              <div className="flex shrink-0 flex-wrap justify-end gap-1.5">
                <StatusPill option={option} />
                <span className="rounded-full border border-white/10 bg-white/5 px-2 py-0.5 font-mono text-[8px] font-bold uppercase text-gray-400">
