...
// ─── Full-app test server (all routes needed for E2E) ────────────────────────
...
async function startE2EServer(): Promise<void> {
  ...
  await new Promise<void>(resolve => {
    e2eServer = createServer(app).listen(0, '127.0.0.1', resolve);
  });
  e2eBaseUrl = `http://127.0.0.1:${(e2eServer!.address() as { port: number }).port}`;
}

async function stopE2EServer(): Promise<void> {
  if (!e2eServer) return;
  await new Promise<void>((resolve, reject) =>
    e2eServer!.close(err => (err ? reject(err) : resolve())),
  );
  e2eServer = null;
  e2eBaseUrl = '';
}
...
describe.skipIf(SKIP_INTEGRATION)('E2E Scenario 1 — Journal ingestion pipeline', () => {
  ...
  beforeAll(async () => {
    ...
    const gr = await fetch(`${e2eBaseUrl}/api/goals`, {
      method: 'POST',
  ...
  afterAll(async () => {
    if (entryId) await fetch(`${e2eBaseUrl}/api/journal/${entryId}`, { method: 'DELETE' });
    if (goalId) await fetch(`${e2eBaseUrl}/api/goals/${goalId}`, { method: 'DELETE' });
    await stopE2EServer();
  });

  it('Step 1 — Create journal entry returns id and pending status', async () => {
    const today = new Date().toISOString().slice(0, 10);
    const res = await fetch(`${e2eBaseUrl}/api/journal`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  it('Step 3 — GET /api/journal/:id/links returns array (may be empty before ingestion completes)', async () => {
    const res = await fetch(`${e2eBaseUrl}/api/journal/${entryId}/links`);
    expect(res.ok).toBe(true);
  ...
  it('Step 4 — POST /api/journal/:id/ingest triggers re-ingestion (idempotent)', async () => {
    const res = await fetch(`${e2eBaseUrl}/api/journal/${entryId}/ingest`, { method: 'POST' });
  ...
  it('Step 5 — GET /api/journal/:id returns updated entry', async () => {
    const res = await fetch(`${e2eBaseUrl}/api/journal/${entryId}`);
    expect(res.ok).toBe(true);
...
describe.skipIf(SKIP_INTEGRATION)('E2E Scenario 2 — Schedule preview and proposals', () => {
  ...
  beforeAll(async () => {
    ...
    const tr = await fetch(`${e2eBaseUrl}/api/tasks`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        goal_id: goalId,
  ...
  afterAll(async () => {
    if (taskId) await fetch(`${e2eBaseUrl}/api/tasks/${taskId}`, { method: 'DELETE' });
    if (goalId) await fetch(`${e2eBaseUrl}/api/goals/${goalId}`, { method: 'DELETE' });
    await stopE2EServer();
  });

  it('GET /api/ai/schedule-preview returns days array and scheduler_result', async () => {
    const res = await fetch(`${e2eBaseUrl}/api/ai/schedule-preview`);
    expect(res.ok).toBe(true);
  ...
  it('GET /api/ai/proposals returns array', async () => {
    const res = await fetch(`${e2eBaseUrl}/api/ai/proposals`);
    expect(res.ok).toBe(true);
...
describe.skipIf(SKIP_INTEGRATION)('E2E Scenario 3 — Graph navigation', () => {
  ...
  it('GET /api/graph returns nodes and edges arrays', async () => {
    const res = await fetch(`${e2eBaseUrl}/api/graph`);
    expect(res.ok).toBe(true);
  ...
  it('graph goal nodes can be fetched individually via /api/goals', async () => {
    ...
    const goalRes = await fetch(`${e2eBaseUrl}/api/goals/${rawId}`);
  ...
  it('GET /api/graph?goal_id= filters to that goal subgraph', async () => {
    const res = await fetch(`${e2eBaseUrl}/api/graph?goal_id=${goalId}`);
    expect(res.ok).toBe(true);
